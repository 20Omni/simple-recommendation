...
def movie_card(row, watched_list, username, section, reason=None, show_button=True, signup_genres=None):
    ...
    st.markdown(f"""
    <style>
    .movie-card{{border:1.5px solid {border};border-radius:16px;padding:16px;
    ...
    .movie-rating{{font-size:1.2rem;color:{rating_c};margin-bottom:6px;}}
...
def genre_selection_page():
    ...
    .genre-card{display:flex;flex-direction:column;align-items:center;justify-content:center;
        ...
        border:2.2px solid #dadbe7;font-size:1rem;cursor:pointer;transition:0.12s;
