...
# ===== Card Renderer (unchanged, skipped here for brevity) =====
# ... keep your existing movie_card() and render_cards() functions ...
...
def dashboard_page():
    ...
    # ===== New Header Banner =====
    fav_genres = st.session_state.genres if st.session_state.genres else []
    genre_chips = "".join([f"<span class='genre-chip'>{g}</span>" for g in fav_genres])

    st.markdown(f"""
    <div style="background: linear-gradient(90deg,#ff4b4b,#ff944b);
                padding:25px;border-radius:15px;margin-bottom:25px;text-align:center;color:white;">
        <h1 style="margin:0;font-size:38px;font-weight:700;">🍿 Welcome, {st.session_state.username}!</h1>
        <p style="margin:10px 0 0;font-size:18px;">Your Favorite Genres:</p>
        <div style="margin-top:8px;">{genre_chips if genre_chips else '<i>No genres selected</i>'}</div>
    </div>
    """, unsafe_allow_html=True)
